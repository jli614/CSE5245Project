...
def get_jaccard_dict(graph):
    ...
    for e in edges:
        ...
        for aa in neigh_a:
            w = graph.get_weight((a, aa))
            neigh_weight_a[aa] = w

        for bb in neigh_b:
            w = graph.get_weight((b, bb))
            neigh_weight_b[bb] = w

        #print(neigh_weight_a)
        #print(neigh_weight_b)

        union_keys = neigh_b.union(neigh_a)
        #print(union_keys)

        union_neigh_weight_a = []
        union_neigh_weight_b = []
        for k in union_keys:
            if k in neigh_weight_a:
                union_neigh_weight_a.append(neigh_weight_a[k])
            else:
                if k == a:
                    union_neigh_weight_a.append(1.0)
                else:
                    union_neigh_weight_a.append(0.0)

            if k in neigh_weight_b:
                union_neigh_weight_b.append(neigh_weight_b[k])
            else:
                if k == b:
                    union_neigh_weight_b.append(1.0)
                else:
                    union_neigh_weight_b.append(0.0)

        #print(union_neigh_weight_a)
        #print(union_neigh_weight_b)

        #0.5*((norm((x-mean(x))-(y-mean(y)))^2)/(norm(x-mean(x))^2+norm(y-mean(y))^2))
        #0.5*(std(x-y)^2) / (std(x)^2+std(y)^2)

        np_a = numpy.asarray(union_neigh_weight_a)
        np_b = numpy.asarray(union_neigh_weight_b)

        normalized_euclidean_distance = \
        0.5 * ((numpy.std(np_a - np_b)**2) / 
               ((numpy.std(np_a))**2 + (numpy.std(np_b))**2))

        #print(normalized_euclidean_distance)

        jaccs[a].append(normalized_euclidean_distance)
        jaccs[b].append(normalized_euclidean_distance)
    return jaccs
